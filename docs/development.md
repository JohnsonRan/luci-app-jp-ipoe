...
### Isolated NFQUEUE experiment

[`snat-queue-path.py`](../tests/experiments/snat-queue-path.py) is a data-path
experiment, **not a deployable allocator or part of the installed package**.
It reuses the collision fixture and picks known test ports beyond a full
segment; it does not discover free ports from live conntrack state.

With the same isolated Linux prerequisites:

```sh
uv run --no-project --offline --python /usr/bin/python3 tests/experiments/snat-queue-path.py
uv run --no-project --offline --python /usr/bin/python3 tests/experiments/snat-queue-path.py --fallback
```

Default mode tests NFQUEUE, a one-shot tuple-keyed selection map and native NAT.
Consumer exit fails closed for new connections while established TCP survives.
`--fallback` places the queue before the current segmented NAT rules:

- No consumer: packets bypass into segmented NAT.
- Full queue: new overflow packets fall back; already-held packets stay queued.
- Responsive timeout: an explicit ACCEPT verdict releases the held SYN.
- Consumer exit with a held SYN: that SYN is dropped; TCP retransmission can
  use fallback.
- Reserved/out-of-pool choices are rejected and consumed before fallback;
  existing TCP mappings and traffic are preserved in the tested cases.

**A hung consumer has no automatic kernel packet deadline.** There is no
watchdog, general concurrent allocator, UDP/ICMP allocation guarantee, or
hot-update guarantee. Fallback still has the selected-segment capacity limit.
